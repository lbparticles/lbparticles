...
class precomputer:
    def __init__(self, timeorder, shapeorder, psir, etarget, nchis, nks, alpha, vwidth=50):
        ...
        for i in range(self.N):
            xCart = [R, 0, 0]
            vCart = [1.0, vs[i], 0]
            part = particle(xCart, vCart, self.psir,
        ...
        self.target_data = np.zeros((nchis, self.N,
                                     # tbd how to interpolate this. rn I'm thinking of doing a 1D interpolation at each chi, then leave the particle to decide how it will interpolate that.
                                     timeorder + 2))
    ...
    def generate_interpolators(self):
        ...
        self.interpolators = np.zeros(
            (self.ordertime + 2, self.nchis), dtype=object)
        self.interpolators_nuphase = np.zeros(
            (self.ordertime + 2, self.nchis), dtype=object)
        for i in range(self.ordertime + 2):
...
class particle:
    ...
    def __init__(self,
        ...
        t_terms, nu_terms = lbdata.get_t_terms(
            self.e, maxorder=self.ordertime+2, includeNu=(zopt == 'first' or zopt == 'zero'), nchis=nchis)
        ...
        mytfac = self.Ubar ** nuk / \
            (self.h * self.m0 * self.ubar * np.sqrt(1 - self.e * self.e))
        nufac = nu0 * tfac * \
        ...
        if zopt == 'first':
            ...
            integrands = np.sin(chi_eval) / (1.0 - self.e *
                                             np.cos(chi_eval)) * np.cos(2.0 * nuu * nufac)
            ...
            integrands = np.sin(chi_eval) / (1.0 - self.e *
                                             np.cos(chi_eval)) * np.sin(2.0 * nuu * nufac)
            to_integrate = scipy.interpolate.CubicSpline(chi_eval, integrands)
    ...
    def initialize_z(self, atol=1.0e-8, rtol=1.0e-8, Neval=1000):
        ...
        res0 = scipy.integrate.solve_ivp(to_integrate, [np.min(ts), np.max(
            ts)], ic0, t_eval=ts, atol=1.0e-7, rtol=1.0e-7, method='DOP853')
        res1 = scipy.integrate.solve_ivp(to_integrate, [np.min(ts), np.max(
            ts)], ic1, t_eval=ts, atol=1.0e-7, rtol=1.0e-7, method='DOP853')
    ...
    def zvz(self, t):
        ...
        # time since reference pericenter. When the input t is 0, we want this to be the same as the tPeri we found in init()
        tPeri = t + self.tperiIC
        # nu times t THIS is phi(t), eq. 27 of Fiore 2022.
...
#            return z,w
            ...
            raise Exception(
                "Need to specify an implemented zopt. Options: integrate, first, zeroeth, tilt")
    ...
    def chi_given_tperi(self, t):
        ...
        res = scipy.optimize.newton(
            to_zero, t * 2 * np.pi / self.Tr, fprime=to_zero_prime, tol=1.0e-12)
        return res
    ...
    def eta_given_chi(self, chi):
        ...
        eta_from_arccos = np.arccos(
            # returns a number between 0 and pi
    ...
    def rphi(self, t):
        ...
        # mphi/em is the angle to the reference pericenter.
...
class _particle:
    ...
    def __init__(self, xCartIn, vCartIn, psir, nunought, lbpre, rnought=8100.0, ordershape=1, ordertime=1, tcorr=True,
        ...
        timezeroes = coszeros(self.ordertime)
        wt_arr = np.zeros((self.ordertime, self.ordertime))
        # the values of w[chi] at the zeroes of cos((n+1)chi)
        wtzeroes = np.zeros(self.ordertime)
        nuk = 2.0 / self.k - 1.0
        for i in range(self.ordertime):
            # coefficient for w0, w1, ... for this zero
        ...
        t_terms, nu_terms = lbpre.get_t_terms(
            self.e, maxorder=self.ordertime+2, includeNu=(zopt == 'first' or zopt == 'zero'), nchis=nchis)
        ...
        # should probably just call mphi instead of doing this
        self.phiIC = self.phi(self.etaIC)

        # do some quick checks:
        condA = np.isclose(self.ubar * (1.0 + self.e *
                           np.cos(self.etaIC)), R ** -self.k)

        assert condA

        # where the particle is in the "global" cylindrical coordinate system. I think we just have to move to add or subtract phiIC and thetaIC where appropriate.
        self.thetaIC = theta
...
def buildlbpre(nchis=1000, nks=100, etarget=0.08, psir=logPotential(220.0), shapeorder=100, timeorder=10, alpha=2.2,
               filename=None):
    if filename == None:
        lbpre = precomputer(timeorder, shapeorder, psir,
                            etarget, nchis, nks, alpha, vwidth=20)
        return lbpre
